(function() {
  ...
  describe('Min-Max Numbers in a List: ', function () {
    ...
    describe('Return the min and max number in the list in a new list follows `[min, max]`', function () {
      ...
      it('should return [77, 78] for [78, 77]', function () {
        expect(myApp.findMinMax([78, 77])).to.eql([77, 78]);
      });

      it('should return [12, 21] for [21, 12]', function () {
        expect(myApp.findMinMax([21, 12])).to.eql([12, 21]);
      });
    ...
    describe('Return the number in the list in a new list follows `[min]` when the number is the min and max number in that list', function () {
      ...
      it('should return [0] for [0, 0, 0, 0]', function () {
        expect(myApp.findMinMax([0, 0, 0, 0])).to.eql([0]);
      });
